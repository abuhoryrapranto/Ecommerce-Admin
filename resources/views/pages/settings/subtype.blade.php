...
@section('page_header', 'All Sub Types')
@section('add_button')
<button type="button" class="btn btn-outline-success btn-sm" data-toggle="modal" data-target="#subTypeModal">
Add New
</button>
@endsection
@section('content')
...
<div class="table-responsive">
    <table class="table table-bordered table-hover">
        ...
        <tbody>
          @if(count($subtypes) > 0)
            @foreach($subtypes as $key => $row)
                <tr>
                    <th scope="row">{{$subtypes->firstItem() + $key}}</th>
                    <td>{{$row->name}}</td>
                    ...
                    <td>
                        <div class="dropdown">
                            ...
                            <div class="dropdown-menu bg-dark" aria-labelledby="dropdownMenuButton">
                                @if($row->status == 1)
                                    <a class="dropdown-item text-danger" href="{{url('setting/statuschange-subtype/'.$row->id.'/0')}}">Deactive</a>
                                @else
                                    <a class="dropdown-item text-success" href="{{url('setting/statuschange-subtype/'.$row->id.'/1')}}">Active</a>
                                @endif
                              
                              <a class="dropdown-item text-danger" href="{{url('setting/delete-subtype/'.$row->id)}}">Delete</a>
                            </div>
                        </div>
                    </td>
                </tr>
        ...
        <div class="modal-body">
          <form action="{{url('setting/save-subtype')}}" method="POST">
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" name="sub_type_name" placeholder="Enter Type Name">
                <div class="form-group">
